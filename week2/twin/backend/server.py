...
def _llm_client_and_model() -> tuple[OpenAI, str]:
    api_key = os.getenv("OPENROUTER_API_KEY")
    if not api_key:
        raise RuntimeError("Set OPENROUTER_API_KEY in backend/.env (or Lambda env)")
    referer = os.getenv("OPENROUTER_HTTP_REFERER", "http://localhost:3000")
    title = os.getenv("OPENROUTER_APP_TITLE", "AI Digital Twin")
...
async def chat(request: ChatRequest):
    try:
        ...
        # Build messages for the LLM (context from context.prompt())
